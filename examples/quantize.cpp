...
static bool sam3_quantize_model(const std::string & fname_inp,
    ...
    fprintf(stderr, "%s: version %d, ftype %d -> %d (%s), %d tensors\n",
            __func__, version, ftype, ftype_out, ggml_type_name(qtype), n_tensors);

    // ── Read + write hparams (copy through) ─────────────────────────────────
    // Mirrors sam3_load_hparams field order exactly.
