...
sam3_video_info sam3_get_video_info_vfe(const vfe::VideoFrameExtractor& extractor) {
	...
	info.fps = (extractor.getDurationSec() > 0) ? static_cast<float>(extractor.getFrameCount()) / static_cast<float>(extractor.getDurationSec()) : 0.0f;
	info.n_frames = static_cast<int>(extractor.getFrameCount());
