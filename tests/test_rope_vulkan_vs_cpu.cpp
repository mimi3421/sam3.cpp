 ...
 *   ./test_rope_vulkan_vs_cpu --compare [cpu|metal|vulkan] [cpu|metal|vulkan]
 */
...
bool test_rope(ggml_backend_t backend, const std::string& backend_name,
    ...
    ggml_backend_tensor_get(Q_out_temp, Q_out.data(),
                             0, Q_out.size() * sizeof(float));
    ggml_backend_tensor_get(K_out_temp, K_out.data(),
                             0, K_out.size() * sizeof(float));
...
int main(int argc, char* argv[]) {
    ...
    if (compare_mode) {
        ...
        if (!output_file.empty()) {
            fprintf(stdout, "\nSaving outputs to %s\n", output_file.c_str());
            save_tensor(Q_out1, output_file + "_Q.bin");
            save_tensor(K_out1, output_file + "_K.bin");
        }
